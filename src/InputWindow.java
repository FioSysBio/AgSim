...
public class InputWindow {
  ...
  private void initialize() {
    ...
    panel.setBorder(new TitledBorder(UIManager.getBorder("TitledBorder.border"), "Cell", TitledBorder.LEADING,
    ...
    bacList.setBorder(new TitledBorder(UIManager.getBorder("TitledBorder.border"), "Bacteria List",
      ...
      public void actionPerformed(ActionEvent arg0) {
        ...
        try {
          ...
          if (radiusField.isEnabled()) {
            r_bac.add(
                Double.parseDouble(radiusField.getText()) * (Math.cbrt(bacteria_scale.get(bacteria_scale.size() - 1))));
          } else {
            r_bac.add(0.0);
          }
          if (rdbtnCocci.isSelected()) {
            ...
            l_bac.add(
                Double.parseDouble(lengthField.getText()) * (Math.cbrt(bacteria_scale.get(bacteria_scale.size() - 1))));
          }
          if (l_bac.get(l_bac.size() - 1) == 0.0) {
            v_bac.add((4 / 3) * Math.PI * Math.pow(r_bac.get(r_bac.size() - 1), 3) * Math.pow(10, -18));
          } else {
            v_bac.add(
                Math.PI * Math.pow(r_bac.get(r_bac.size() - 1), 2) * l_bac.get(l_bac.size() - 1) * Math.pow(10, -18));
          }
          if (massField.getText() == "") {
            m_bac.add(1.1 * v_bac.get(v_bac.size() - 1) * Math.pow(10, 6));
          } else {
            m_bac.add(Double.parseDouble(massField.getText()) * Math.pow(10, -12)
                * bacteria_scale.get(bacteria_scale.size() - 1));
          }
    ...
    bacNameComboBox.setModel(new DefaultComboBoxModel(new String[] { "E. coli", "B. subtilis", "S. cervisiae",
        "S. stipitis", "B. adolescentis", "F. prausnitzii", "C. beijerinckii", "Custom..." }));
    ...
    metNamecomboBox.setModel(new DefaultComboBoxModel(
        new String[] { "Glucose", "Acetate", "Butyrate", "Formate", "Ethanol", "Glycerol", "Custom..." }));
    ...
    metaboliteList.setBorder(new TitledBorder(UIManager.getBorder("TitledBorder.border"), "Metabolite List",
        TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
    ...
    panel_4.setBorder(new TitledBorder(UIManager.getBorder("TitledBorder.border"), "Feeding Strategy",
        TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
    ...
    feedlist.setBorder(new TitledBorder(UIManager.getBorder("TitledBorder.border"), "Feeding Points List",
        TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
    ...
    feedButton.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent arg0) {
        try {
          ...
          if (coord[0] <= Integer.parseInt(envLField.getText()) && coord[1] <= Integer.parseInt(envWField.getText())
              && coord[2] <= Integer.parseInt(envWField.getText())) {
            feeding_points.add(coord);
            dlm3.addElement(xFeedField.getText() + ", " + yFeedField.getText() + ", " + zFeedField.getText());
          } else {
            JOptionPane.showMessageDialog(getFrame(), "input must be within environment size. Try Again", "Error",
    ...
    btnNext.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ...
        if (bacteria_name.isEmpty()) {
          JOptionPane.showMessageDialog(getFrame(), "No bacteria! Please add some.", "Error",
              JOptionPane.ERROR_MESSAGE);
          return;
        }
        ...
        try {
          ...
          n_real = Double.parseDouble(metScaleField1.getText())
              * Math.pow(10, Integer.parseInt(metScaleField2.getText()));
  ...
  public void cToN() {
    ...
    for (int i = 0; i < bacteria_conc.size(); i++) {
      if (bacteria_conc.get(i) != -1) {
        // System.out.println(bacteria_conc.get(i)+
        // ","+m_bac.get(i)+","+bacteria_scale.get(i));
