...
public class Bacteria extends Entity {
  ...
  // this value has chosen depended on Gaussian distribution witch mean value
  // equals eat_rate and sigma = 1/3*eat_rate
  ...
  public void SetProperty() {
    ...
    setColor(Environment.bacteria_color.get(type).getRed(), Environment.bacteria_color.get(type).getGreen(),
        Environment.bacteria_color.get(type).getBlue());
  ...
  public void draw(Graphics g) {
    ...
    if (l_bac == 0) {
      g2d.fillOval((int) (getX() / Environment.getTickX()), (int) (getY() / Environment.getTickY()), 2 * getSizeX(),
          2 * getSizeX());
    } else {
      g2d.fillRoundRect((int) (getX() / Environment.getTickX()), (int) (getY() / Environment.getTickY()),
          2 * getSizeX() + 2, getSizeX() + 1, 80, 100);
    }
  ...
  // void to calculate distance between two objects and compare it with set
  // distance
  ...
  // double v = c/(x*(t_d/Environment.getNorm()))*Math.pow(10, 3); //unit =
  // [mmol/gr.hr]
  // return v;
  // }
  ...
  public void divide(List<Entity> le1) {
    ...
    for (int i = 0; i < count - 1; i++) {
      Bacteria b = new Bacteria((int) (getX() + getSizeX() / 2 * getDx()), (int) (getY() + getSizeX() / 2 * getDy()),
          (int) (getZ() + getSizeX() / 2 * getDz()), this.type);
      b.setMass(m_dummy);
  ...
  public void updateIndexes() {
    ...
    for (int i = 1; i < Environment.metabolite_count.size(); i++) {
      Environment.metabolite_index.set(i,
          Environment.metabolite_index.get(i - 1) + Environment.metabolite_count.get(i - 1));
    }
  ...
  public ArrayList<Double> eat(List<Entity> PS, int i) {
    ...
    for (int j = 0; j < PS.size(); j++) {
      ...
      if (CheckDistance(pol.getX(), this.getStepX(), pol.getY(), this.getStepY(), pol.getZ(), this.getStepZ(),
          eatRadius)) {
    ...
    for (int k = v_out.size() - 2; k >= 0; k--) {
      if (exRxnsDirection.get(k) == 0) {
        ...
        // PS.remove(index);
        // }
        // Environment.metabolite_count.set(k, Environment.metabolite_count.get(k) -
        // nearMetabolites[k].size());
  ...
  public void wall() {
    ...
    while ((this.getStepX() < 0) || (this.getStepX() + l > Environment.getL()) || (this.getStepY() < 0)
        || (this.getStepY() + 2 * r_bac > Environment.getW()) || (this.getStepZ() < 0)
        || (this.getStepZ() + 2 * r_bac > Environment.getD())) {
      if (this.getStepX() < 0) {
  ...
  public void tick(List<Entity> PS, java.util.List<Entity> A,
    ...
    if (index != -1) {
      ...
      // and moves to it
      if (CheckDistance(ps.getX(), getX(), ps.getY(), getY(), ps.getZ(), getZ(),
          (getSpeed() / Environment.getNorm()) * Environment.getTickTime())) {
        setStepX(ps.getX());
        setStepY(ps.getY());
        setStepZ(ps.getZ());
      } else {
